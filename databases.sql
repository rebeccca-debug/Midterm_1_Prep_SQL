...
/*1: What is the average population of all the states?*/
SELECT AVG(population) FROM States S;

/*2: Which states have a population greater than the average of all the states?*/
SELECT S.statename from States S where S.population > (SELECT AVG(population) from States);

/*3: How many states are there?*/
SELECT count(*) from States;

/*4: What is the total population of the United States (according to this database)?*/
SELECT SUM(population) FROM States;

/*5: List each state that is below the average state population and give that state's amount below the average.*/
...
/*6: What is the alphabetically first politician by their last name?*/
SELECT DISTINCT P.lastname from Politicians P
ORDER BY P.lastname limit 1;

/*7: What is the alphabetically last politician by their last name?*/
SELECT max(P.lastname) from Politicians P limit 1;

/*8: What is the last names of politicians who are alphabetically first and last?*/
...
/*9: What is the last names of the politicians who are alphabetically first and last?
...
/*10: What is the last name of the alphabetically second through tenth politician?*/
...
/*11: Find the name of sailors who have reserved all boats.*/
...
/*12: Find the names of sailors who have reserved all boats whose name have 'lake' in it.*/
...
/*13: Find the sailor id's of sailors whose rating is better than a sailor named Bart.*/
SELECT S.sid from Sailors S WHERE S.rating > (SELECT S.rating from Sailors S WHERE S.sname='Bart');

/*14: Find the name and age of the oldest sailor.*/
